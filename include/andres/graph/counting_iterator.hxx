#ifndef ANDRES_GRAPH_COUNTING_ITERATOR_HXX
#define ANDRES_GRAPH_COUNTING_ITERATOR_HXX
...
//@{
    ...
    /** \brief Iterator that counts upwards or downwards with a given step size.
        ...
        <b>\#include</b> \<andres/graph/counting_iterator.hxx\><br>
                       ...
                       T, std::ptrdiff_t, T const *, T>
{
  public:
    ...
    , step_(step)
    {
        GRAPH_CHECK_OP(step, !=, 0,
            "CountingIterator(): step must be non-zero.");
        GRAPH_CHECK((step > 0 && begin <= end) || (step < 0 && begin >= end),
